...
<<<<<<< HEAD
    ...
    * 업로드 가능한 인증서는 최대 512KB 입니다.
6. **패스프레이즈**에  인증서 파일 내에 포함된 개인 키의 패스프레이즈(passphrase, 비밀문구)를 입력합니다.
7. **추가** 버튼을 클릭합니다.
8. [Network > Load Balancer](https://beta-gov.toast.com/kr/service/network/load-balancer) 상품과 연동이 필요한 경우, 등록하는 인증서 파일에 패스프레이즈(passphrase, 비밀문구)는 제거되어야 합니다.
    * 패스프레이즈(passphrase, 비밀문구) 제거를 위해서는 다음 명령을 사용하여 제거할 수 있습니다.
    ```bash
...
6. **패스프레이즈**에  인증서 파일 내에 포함된 개인 키의 패스프레이즈(passphrase, 비밀문구)를 입력합니다.
7. **추가** 버튼을 클릭합니다.
8. [Network > Load Balancer](https://alpha-gov.toast.com/kr/service/network/load-balancer) 상품과 연동이 필요한 경우, 등록하는 인증서 파일에 패스프레이즈(passphrase, 비밀문구)는 제거되어야 합니다.
    * 패스프레이즈(passphrase, 비밀문구) 제거를 위해서는 다음 명령을 사용하여 제거할 수 있습니다.
``` bash
openssl rsa -in my_private_input.key -out my_private_output.key
```
